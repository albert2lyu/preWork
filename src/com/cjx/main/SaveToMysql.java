...
public class SaveToMysql {
	...
	{
		File file = new File("E:/cjx/patents/patentPages/"+type);
	...
	private PatentDao parseHtmlToPatentDao(String html,String type) throws ParserException
	{
		PatentDao pttDao = new PatentDao();
		pttDao.setPttType(type);
		Parser parser = new Parser(html.toString());
		
		NodeList nodeList1 = parser.extractAllNodesThatMatch(new HasAttributeFilter("style","table-layout:fixed;word-break:break-all;word-wrap:break-word"));
		...
		pttDao.setApplyNum(colAry[1].getStringText().replace("&nbsp;<span class=\"zi_10\">", "").replace("</span>", ""));
		pttDao.setApplyDate(Date.valueOf(colAry[3].getStringText().replace("&nbsp;","").replace(".", "-")));
		...
		pttDao.setPttDate(Date.valueOf(colAry[3].getStringText().replaceAll("&nbsp;|\\s","").replace(".", "-")));
		
		row = tableTag.getRow(3);
		colAry = row.getColumns();
		pttDao.setPttMainClassNum(colAry[1].getStringText().replaceAll("&nbsp;|\\s",""));
		
		row = tableTag.getRow(4);
		colAry = row.getColumns();
		pttDao.setPttClassNum(colAry[1].getStringText().replaceAll("&nbsp;|\\s",""));
		...
		colAry = row.getColumns();
		pttDao.setProposerAddress(colAry[1].getStringText().replaceAll("&nbsp;|\\s",""));
		
		row = tableTag.getRow(8);
		colAry = row.getColumns();
		pttDao.setInventor(colAry[1].getStringText().replaceAll("&nbsp;|\\s",""));
		pttDao.setInternationalApply(colAry[3].getStringText().replaceAll("&nbsp;|\\s",""));
		
		row = tableTag.getRow(9);
		colAry = row.getColumns();
		pttDao.setInternationalPublication(colAry[1].getStringText().replaceAll("&nbsp;|\\s",""));
		if(!colAry[3].getStringText().replaceAll("&nbsp;|\\s|;","").replace(".", "-").equals(""))
		{
			pttDao.setIntoDate(Date.valueOf(colAry[3].getStringText().replaceAll("&nbsp;|\\s|;","").replace(".", "-")));
		}
		else
		{
		...
		colAry = row.getColumns();
		pttDao.setPttAgencyOrg(colAry[1].getStringText().replaceAll("&nbsp;|\\s",""));
		pttDao.setPttAgencyPerson(colAry[3].getStringText().replaceAll("&nbsp;|\\s",""));
		
		parser = new Parser(html.toString());
		NodeList nodeList2 = parser.extractAllNodesThatMatch(new HasAttributeFilter("class","zi_zw"));
